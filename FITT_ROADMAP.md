...
12. **Adapt the feature set to the model you can run.** FITT must stay usable on the local models a user can actually host, which span a wide capability range — so features behave like a game's graphics settings, not an all-or-nothing gate. The flat tool-calling loop is the floor every model degrades to; the richer features (planning/orchestration, deeper memory, longer context) are higher settings a more capable model unlocks. Probes and benchmarks (the capability profile) *measure* where a model sits; a separate advisory layer *recommends* which features to enable for it; the operator confirms. The recommendation is never auto-applied and never a single Low/Med/High slider — capability is multi-dimensional and sample-noisy (a model can ace tool-calling and fail planning), so the read is per-feature and advisory, in the spirit of Principle 11. Enabling a feature the bound model can't drive — planning on a model that elects to plan 0% of the time — is a misconfiguration to surface, not a silently dead feature.
...
*Full spec: `.kiro/specs/phase5-lessons/`. Spec promoted 2026-05-08; implementation shipped 2026-05-08 (lessons + `learn_*` tools + `fitt learn` CLI + tool-turn structured persistence + decaying history injection + background history pruner). The strict-xfail session-poisoning e2e test flipped green unassisted — the Phase 5 binary gate. DONE 2026-07-02: validation reconciled to automated tests (spec section 13) rather than manual live checks — the lessons + session-poisoning e2e lifecycles, the decay unit suite plus a new `test_history_decay_lifecycle.py` (decayed history reaches the dispatched prompt over HTTP), and the history-pruner unit + dashboard-surface tests together cover the original 13a–13f live steps. No live check pending.*

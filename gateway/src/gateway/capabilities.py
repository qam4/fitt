...
def is_tool_use_expected_but_none(
    ...
    **Scope (post-2026-05-12): eval / probe contexts only.**
    The function has no way to know whether the user's
    request actually needed a tool. In a test harness the
    expected outcome is pinned by the test author (either
    ``expected_tool`` in an :class:`EvalCase` or the
    per-canary contract of :mod:`alias_probe`); in live
    chat, it isn't. Live use confirmed the false-positive
    rate is 100% for ordinary chit-chat: every "thanks"
    / "who are you" reply in Telegram tripped this check
    as "model declined to call a tool when one was
    expected" even though the caller hadn't asked for one.
    We pulled the live-chat and cron callers on 2026-05-12;
    see ``docs/observed-issues.md``. The function stays as
    the pass/fail classifier for :mod:`alias_probe` and
    :mod:`alias_eval`, where the expected tool is specified
    by the test case.

    Does NOT catch:
    * Models that correctly decline because no listed tool
      actually applies. Short-reply threshold handles the
      common case; true gap phrasing is handled by the
      gap-reporter path separately (see :func:`parse_gap`).
